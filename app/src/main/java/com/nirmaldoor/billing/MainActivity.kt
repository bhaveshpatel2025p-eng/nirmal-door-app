...
data class Door(var name:String="",var widthCm:Double=0.0,var heightCm:Double=0.0,var qty:Double=1.0,var rate:Double=0.0){
    fun sqft()=(widthCm*heightCm)/929.0304
...
class MainActivity:Activity(){
    ...
    override fun onCreate(b:Bundle?){super.onCreate(b);setContentView(R.layout.activity_main)
        ...
        newBill();findViewById<Button>(R.id.addDoor).setOnClickListener{doors.add(Door());render()}
        findViewById<Button>(R.id.save).setOnClickListener{saveBill()};findViewById<Button>(R.id.pdf).setOnClickListener{pdf()}
        findViewById<Button>(R.id.whatsapp).setOnClickListener{whatsapp()};findViewById<Button>(R.id.newBill).setOnClickListener{newBill()}
        listOf(discount,gst,paid).forEach{it.watch{calc()}}
    }
    fun newBill(){inv.setText("ND-${SimpleDateFormat("yyyy",Locale.US).format(Date())}-${System.currentTimeMillis().toString().takeLast(6)}");customer.setText("");mobile.setText("");address.setText("");discount.setText("0");gst.setText("0");paid.setText("0");doors.clear();doors.add(Door());render()}
    fun render(){container.removeAllViews();doors.forEachIndexed{idx,d->
        val box=LinearLayout(this);box.orientation=LinearLayout.VERTICAL;box.setPadding(14,14,14,14);box.setBackgroundColor(0xFFFFFFFF.toInt())
        val title=TextView(this);title.text="DOOR ${idx+1}";title.textSize=16f;title.setTypeface(null,1);box.addView(title)
        fun field(h:String,v:String,change:(Double)->Unit):EditText{val e=EditText(this);e.hint=h;e.setText(if(v=="0.0")"" else v);e.inputType=2;e.watch{change(it.toDoubleOrNull()?:0.0);calc()};return e}
        val name=EditText(this);name.hint="Door / Item name";name.setText(d.name);name.watch{d.name=it}
        box.addView(name);box.addView(field("Width (cm)",d.widthCm.toString()){d.widthCm=it});box.addView(field("Height (cm)",d.heightCm.toString()){d.heightCm=it});box.addView(field("Quantity",d.qty.toString()){d.qty=it});box.addView(field("Rate / Sq.Ft (₹)",d.rate.toString()){d.rate=it})
        val out=TextView(this);out.textSize=15f;out.setPadding(4,10,4,10);box.addView(out)
        val rm=Button(this);rm.text="REMOVE DOOR";rm.setOnClickListener{doors.removeAt(idx);render()};box.addView(rm);container.addView(box)
    };calc()}
    fun calc(){val sub=doors.sumOf{it.amount()};val dis=discount.text.toString().toDoubleOrNull()?:0.0;val taxable=(sub-dis).coerceAtLeast(0.0);val tax=taxable*(gst.text.toString().toDoubleOrNull()?:0.0)/100;val total=taxable+tax;val p=paid.text.toString().toDoubleOrNull()?:0.0;val due=(total-p).coerceAtLeast(0.0);summary.text="Subtotal: ${money(sub)}\nDiscount: ${money(dis)}\nGST: ${money(tax)}\nGRAND TOTAL: ${money(total)}\nPaid: ${money(p)}\nBALANCE DUE: ${money(due)}";for(i in 0 until container.childCount){val box=container.getChildAt(i) as LinearLayout;if(box.childCount>5)(box.getChildAt(5) as TextView).text="Area: %.2f Sq.Ft   |   Amount: %s".format(doors[i].sqft(),money(doors[i].amount()))}}
    fun money(x:Double)=String.format(Locale.US,"₹%,.2f",x)
    fun text():String{val sub=doors.sumOf{it.amount()};val dis=discount.text.toString().toDoubleOrNull()?:0.0;val taxable=(sub-dis).coerceAtLeast(0.0);val tax=taxable*(gst.text.toString().toDoubleOrNull()?:0.0)/100;val total=taxable+tax;val p=paid.text.toString().toDoubleOrNull()?:0.0;val due=(total-p).coerceAtLeast(0.0);return buildString{append("NIRMAL DOOR\nOpp Railway Station, KUDACHI-591311\nMobile: 8050304196\nEmail: nirmaldoorkud@gmail.com\n\nInvoice: ${inv.text}\nCustomer: ${customer.text}\nMobile: ${mobile.text}\nAddress: ${address.text}\n\n");doors.forEachIndexed{i,d->append("${i+1}. ${d.name} | ${d.widthCm} cm x ${d.heightCm} cm | %.2f Sq.Ft x %.0f @ %.2f = %.2f\n".format(d.sqft(),d.qty,d.rate,d.amount()))};append("\nGrand Total: %.2f\nPaid: %.2f\nBalance Due: %.2f".format(total,p,due))}}
